...
function ProjectCard({ project, direction, prefersReducedMotion }: { project: Project; direction: 'left' | 'right'; prefersReducedMotion: boolean }) {
  return (
    <motion.div
      initial={prefersReducedMotion ? { y: 0, opacity: 1 } : { y: 60, opacity: 0 }}
      whileInView={{ y: 0, opacity: 1 }}
      viewport={{ once: false, amount: 0.15 }}
      transition={prefersReducedMotion ? {} : { type: 'spring', stiffness: 110, damping: 13, mass: 1, delay: direction === 'right' ? 0.1 : 0 }}
